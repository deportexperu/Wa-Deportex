...
describe("mondayIndex", () => {
  it("maps Monday → 0 and Sunday → 6", () => {
    expect(mondayIndex(new Date("2026-05-18T12:00:00Z"))).toBe(0); // Mon
    expect(mondayIndex(new Date("2026-05-19T12:00:00Z"))).toBe(1); // Tue
    expect(mondayIndex(new Date("2026-05-23T12:00:00Z"))).toBe(5); // Sat
    expect(mondayIndex(new Date("2026-05-24T12:00:00Z"))).toBe(6); // Sun
  });

  it("aligns with DOW_SHORT_MON_FIRST labels", () => {
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date("2026-05-18T12:00:00Z"))]).toBe(
      "Mon",
    );
    expect(DOW_SHORT_MON_FIRST[mondayIndex(new Date("2026-05-24T12:00:00Z"))]).toBe(
      "Sun",
    );
  });
});
